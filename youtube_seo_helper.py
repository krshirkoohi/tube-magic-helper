#!/usr/bin/env python3
"""
YouTube SEO Helper - Lightweight console YouTube AI assistant
"""
import os
import sys
import json
...
# Initialize console
console = Console()
app = typer.Typer(help="YouTube SEO Helper - YouTube AI Assistant")
...
def research_cmd(
    ...
    suggestions: int = typer.Option(5, "--suggestions", "-s", help="Number of suggestions per seed keyword"),
    ...
    if not kw_list:
        console.print("[red]Please provide at least one seed keyword.[/red]")
        return
        
    console.print(f"[bold]Starting keyword research with seeds: {', '.join(kw_list)}[/bold]")
    results = keyword_research(kw_list, limit, suggestions)
    
    if not results or not results["all_keywords"]:
        console.print("[yellow]No results found.[/yellow]")
        return
    
    # Display summary
    console.print(Panel.fit(
        f"[green]✓ Found {results['stats']['total']} keywords\n"
        f"• Channel-specific: {results['stats']['channel_specific']}\n"
        f"• Category-specific: {results['stats']['category_specific']}\n"
        f"• Video-specific: {results['stats']['video_specific']}",
        title="Keyword Research Summary",
        border_style="green"
    ))
    
    # Display results in a table
    table = Table(title="Top Keywords by Magic Score", show_header=True, header_style="bold magenta")
    table.add_column("Keyword", style="cyan", no_wrap=True)
    table.add_column("Magic Score", justify="right")
    ...
    for result in results["all_keywords"]:
        table.add_row(
            result["keyword"],
            f"{result['magic_score']:.2f}",
            f"{result['avg_views']:,}",
            result["competition"],
            f"{result['engagement_rate']}%",
            f"[{'green' if result['keyword_type'] == 'channel' else 'yellow' if result['keyword_type'] == 'category' else 'blue'}]{result['keyword_type'].capitalize()}[/]"
        )
    ...
    if output:
        try:
            ...
            with open(output, 'w') as f:
                ...
                f.write("======================\n\n")
                f.write(f"Seed Keywords: {', '.join(kw_list)}\n\n")
                
                f.write("Top Keywords by Magic Score:\n")
                for result in results["all_keywords"]:
                    f.write(f"{result['keyword']} - Magic Score: {result['magic_score']:.2f}, "
                           f"Avg Views: {result['avg_views']:,}, Competition: {result['competition']}, "
                           f"Engagement: {result['engagement_rate']}%, Type: {result['keyword_type'].capitalize()}\n")
                
                f.write("Top Keywords by Magic Score:\n")
                f.write("--------------------------\n")
                for result in results["all_keywords"][:20]:  # Top 20 in the text report
                    f.write(f"{result['keyword']} - Magic Score: {result['magic_score']:.2f}, "
                           f"Avg Views: {result['avg_views']:,}, Competition: {result['competition']}, "
                           f"Engagement: {result['engagement_rate']}%, Type: {result['keyword_type'].capitalize()}\n")
                ...
                f.write("-----------------------\n")
                for result in categories['channel'][:10]:
                    f.write(f"{result['keyword']} - Magic Score: {result['magic_score']:.2f}\n")
                    
                f.write("\nCategory-specific Keywords:\n")
                f.write("-----------------------\n")
                for result in categories['category'][:10]:
                    f.write(f"{result['keyword']} - Magic Score: {result['magic_score']:.2f}\n")
                    
                f.write("\nVideo-specific Keywords:\n")
                f.write("---------------------\n")
                for result in categories['video'][:10]:
                    f.write(f"{result['keyword']} - Magic Score: {result['magic_score']:.2f}\n")
    ...
    if export_csv:
        csv_file = output_path / f"keywords_{filename_base}.csv"
        try:
            import csv
            with open(csv_file, 'w', newline='') as f:
                ...
                # Write header
                writer.writerow([
                    "Keyword", "Magic Score", "Avg Views", "Competition", 
                    "Engagement Rate", "Type", "Avg Duration (min)", "Avg Likes", "Avg Comments"
                ])
            ...
            console.print(f"[green]✓ CSV file saved to {csv_file} (ready for Google Sheets import)[/green]")
        except Exception as e:
    ...
    console.print(Panel.fit(
        ...
        "4. Organize into channel, category, and video-specific groups\n"
        "5. Use these keywords in your video titles, descriptions, and tags",
        title="Complete Your Keyword Process",
        border_style="yellow"
    ))

@app.command("keyword-process")
def keyword_process_cmd(
    keywords: str = typer.Argument(..., help="Comma-separated seed keywords"),
    suggestions: int = typer.Option(10, "--suggestions", "-s", help="Number of suggestions per seed keyword"),
    limit: int = typer.Option(50, "--limit", "-l", help="Maximum number of keywords to analyze"),
    output_dir: str = typer.Option("./data", "--output-dir", "-d", help="Directory to save output files"),
    export_csv: bool = typer.Option(True, "--csv", help="Export results as CSV for spreadsheet import")
):
    """Implement the 'secret keyword process' for YouTube SEO.
    
    This command follows the complete workflow:
    1. Research and expand seed keywords
    2. Calculate Magic Score (M = V/C) for each keyword
    3. Categorize keywords (channel, category, video specific)
    4. Export results for further analysis
    
    Results can be exported to CSV for use in Google Sheets or other spreadsheet tools.
    """
    if not check_api_keys():
        return
        
    # Step 1: Process seed keywords
    seed_kw_list = [k.strip() for k in keywords.split(",") if k.strip()]
    if not seed_kw_list:
        console.print("[red]Please provide at least one seed keyword.[/red]")
        return
    ...
    # Generate a filename based on the first few keywords
    filename_base = "_".join(seed_kw_list[:3]).replace(" ", "_").lower()
    if len(seed_kw_list) > 3:
        filename_base += "_and_more"
    
    # Step 2: Perform the keyword research
    console.print(Panel.fit(
        "\n".join([
            "[bold]Secret Keyword Process[/bold]",
            "",
            "1. Research & expand seed keywords",
            "2. Calculate Magic Score (M = V/C)",
            "3. Categorize keywords",
            "4. Export results for analysis"
        ]),
        title="Starting Keyword Process",
        border_style="blue"
    ))
    
    console.print(f"\n[bold]Starting with seed keywords:[/bold] {', '.join(seed_kw_list)}")
    
    # Step 3: Expand and analyze keywords
    with console.status("[bold blue]Expanding keywords...[/bold blue]") as status:
        # First expand all keywords to get suggestions
        all_keywords = set()
        for kw in seed_kw_list:
            kw_lower = kw.lower().strip()
            all_keywords.add(kw_lower)
            status.update(f"[bold blue]Getting suggestions for: {kw_lower}[/bold blue]")
            suggestions_list = _keyword_suggestions(kw_lower, suggestions)
            all_keywords.update(suggestions_list)
    
    # Limit the number of keywords to analyze if needed
    if len(all_keywords) > limit:
        console.print(f"[yellow]Limiting analysis to {limit} keywords (from {len(all_keywords)} total)[/yellow]")
        # Prioritize seed keywords, then take random sample of suggestions
        final_keywords = set(seed_kw_list)
        remaining = list(all_keywords - final_keywords)
        import random
        random.shuffle(remaining)
        final_keywords.update(remaining[:limit - len(final_keywords)])
        all_keywords = final_keywords
    
    console.print(f"[green]Analyzing {len(all_keywords)} keywords...[/green]")
    
    # Step 4: Analyze each keyword
    results = []
    with console.status("[bold blue]Calculating Magic Scores...[/bold blue]") as status:
        for i, kw in enumerate(all_keywords, 1):
            status.update(f"[bold blue]Analyzing keyword {i}/{len(all_keywords)}: {kw}[/bold blue]")
            stats = _keyword_stats(kw)
            if stats:
                results.append(stats)
    
    if not results:
        console.print("[red]No results found. Try different seed keywords.[/red]")
        return
    
    # Step 5: Sort by Magic Score and categorize
    results.sort(key=lambda x: x["magic_score"], reverse=True)
    ...
    console.print(Panel.fit(
        f"[green]✓ Found {len(results)} high-potential keywords\n"
        f"• Channel-specific: {len(categories['channel'])}\n"
        f"• Category-specific: {len(categories['category'])}\n"
        f"• Video-specific: {len(categories['video'])}",
        title="Keyword Analysis Complete",
        border_style="green"
    ))
    
    # Step 7: Display top keywords in a table
    table = Table(
        title="Top 10 Keywords by Magic Score",
        show_header=True,
    ...
    # Step 8: Save results
    # Text report
    report_file = output_path / f"keyword_report_{filename_base}.txt"
    try:
        with open(report_file, 'w') as f:
            f.write(f"Keyword Research Results - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
            f.write("======================\n\n")
            f.write(f"Seed Keywords: {', '.join(seed_kw_list)}\n\n")
            
            f.write(f"Total Keywords Analyzed: {len(results)}\n")
            f.write(f"Channel-specific Keywords: {len(categories['channel'])}\n")
            f.write(f"Category-specific Keywords: {len(categories['category'])}\n")
            f.write(f"Video-specific Keywords: {len(categories['video'])}\n\n")
            
            f.write("Top Keywords by Magic Score:\n")
            f.write("--------------------------\n")
            for result in results[:20]:  # Top 20 in the text report
                f.write(f"{result['keyword']} - Magic Score: {result['magic_score']:.2f}, "
                       f"Avg Views: {result['avg_views']:,}, Competition: {result['competition']}, "
                       f"Engagement: {result['engagement_rate']}%, Type: {result['keyword_type'].capitalize()}\n")
...
def main():
    """YouTube SEO Helper - Lightweight console YouTube AI assistant."""
    console.print(Panel.fit(
        "[bold blue]YouTube SEO Helper[/bold blue]\n"
